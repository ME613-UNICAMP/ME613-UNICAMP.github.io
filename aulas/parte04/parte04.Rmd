...
author: Benilton S Carvalho - 2S2020
output:
...
knitr::kable(cbind(dados[1:5, ], dados[-(1:5),]),
             booktabs=TRUE) %>% 
  kable_styling() %>% 
  add_header_above(c("Parte 1"=2, "Parte 2"=2))
```
...
g + theme_bw() + ggtitle("Treinamento vs. Performance")
```
...
ggplot(dados, aes(x = fit, y = res)) +
  geom_point() + xlab("Fitted") + ylab("Residuals") +
  geom_smooth(method = "loess",se=FALSE) +
  theme_bw() + ggtitle("Resíduos da Regressão")
```
...
```{r,echo=FALSE,fig.align='center', warning=FALSE, message=FALSE}
ggplot(dados, aes(x = Days, y = res)) +
  geom_point() + xlab("Days") + ylab("Residuals") +
  geom_smooth(method = "loess",se=FALSE) +
  theme_bw() + ggtitle("Resíduos da Regressão")
```
...
ggplot(dados, aes(x = sqrt(Days), y = Performance)) +
  geom_point() + xlab(expression(sqrt("Treinamento"))) + ylab("Performance") +
  geom_smooth(method = "lm",se=FALSE) +
  theme_bw() + ggtitle(expression(paste("Regressão de Performance vs. ", sqrt("Treinamento"))))
```
...
Gráfico de resíduos: $e_i$ versus $\hat{Y}_i$ usando $\sqrt{\mbox{Treinamento}}$ no modelo.

```{r,echo=FALSE,fig.align='center', warning=FALSE, message=FALSE}
ggplot(dados, aes(x = fit2, y = res2)) + geom_point() +
...
Gráfico de resíduos: $e_i$ versus $X_i$ usando $\sqrt{\mbox{Treinamento}}$ no modelo.

```{r,echo=FALSE,fig.align='center', warning=FALSE, message=FALSE}
ggplot(dados, aes(x = sqrt(Days), y = res2)) + geom_point() +
  xlab(expression(sqrt(Days))) + ylab("Residuals") +
  geom_smooth(method = "loess", se=FALSE) +
  theme_bw()
```
...
```{r,echo=FALSE}
dados <- read.table("./dados/CH03TA08.txt")[, 1:2]
colnames(dados) <- c("Idade","Poliamina")
knitr::kable(cbind(dados[1:7,], dados[8:14,], dados[15:21,], dados[22:28,])) %>% 
  kable_styling() %>% 
  add_header_above(c("Parte 1"=2, "Parte 2"=2, "Parte 3"=2, "Parte 4"=2))
```
...
library(ggplot2)
ggplot(dados, aes(x = Idade, y = Poliamina)) +
  geom_point() + xlab("Idade") + ylab("Poliamina") +
  geom_smooth(method = "lm", se=FALSE) +
  ggtitle("Poliamina vs. Idade") + theme_bw()
```

$$\hat{Y}=`r coef(modelo)[1]` `r ifelse(coef(modelo)[2] > 0, "+", "-")` `r abs(coef(modelo)[2])`X$$
...
dados$fit <- fitted(modelo)
dados$res <- resid(modelo)
ggplot(dados, aes(x = fit, y = res)) + geom_point() +
  xlab("Fitted") + ylab("Residuals") +
  geom_smooth(method = "loess", se=FALSE) +
  ggtitle("Poliamina vs. Idade") +
  theme_bw()
```
...
dados$fit <- fitted(modelo)
dados$res <- resid(modelo)
ggplot(dados, aes(x = Idade, y = res)) + geom_point() +
  xlab("Idade") + ylab("Residuals") +
  geom_smooth(method = "loess", se=FALSE) +
  ggtitle("Poliamina vs. Idade") + theme_bw()
```
...
ggplot(dados, aes(x = Idade, y = log10(Poliamina))) +
  geom_point() + xlab("Idade") + ylab("Y") +
  geom_smooth(method = "lm",se=FALSE) +
  ggtitle("log10(Poliamina) vs. Idade") +
  theme_bw()
```

$$\hat{Y'}=`r round(coef(modelo2)[1],2)` `r ifelse(coef(modelo2)[2] > 0, "+", "-")` `r abs(round(coef(modelo2)[2],2))`X$$
...
```{r,echo=FALSE,fig.align='center', warning=FALSE, message=FALSE}
ggplot(dados, aes(x = fit2, y = res2)) +
  geom_point() + xlab("Fitted") + ylab("Residuals") +
  geom_smooth(method = "loess", se=FALSE) +
  ggtitle("Poliamina e Idade") + theme_bw()
```
...
```{r,echo=FALSE,fig.align='center', warning=FALSE, message=FALSE}
ggplot(dados, aes(x = Idade, y = res2)) +
  geom_point() + xlab("Idade") + ylab("Residuals") +
  geom_smooth(method = "loess", se=FALSE) +
  ggtitle("Poliamina vs. Idade") + theme_bw()
```
