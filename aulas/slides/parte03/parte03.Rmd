...
```{r,echo=FALSE,fig.hegiht=4,fig.align='center'}
...
com graus de liberdade $n_e=\sum_{j=1}^m(n_j-1)=\sum_{j=1}^mn_j-m$.

$$QME_{puro}=s_e^2=\frac{\sum_{j=1}^m\sum_{u=1}^{n_j}(Y_{ju}-\bar{Y}_j)^2}{\sum_{j=1}^mn_j-m}$$
...
**Mostre$^1$** que a $SQE$ pode ser particinada da seguinte forma:
...
Valores relativamente altos de $SQF_a$ indicam discrepância entre esses dois estimadores e, consequentemente, indicam evidˆncia contra a adequação do modelo de regressão linear simples.
...
```
$s_{e}=`r round(extract2a$sigma.djt,2)`$ enquanto que, usando o modelo anterior, temos que $s=`r round(extract1a$sigma.djt,2)`$.
...
$$F^*=\frac{SQF_a/(m-2)}{SQE_p/(n-m)}=\frac{13594/4}{1148/5}=14.8$$
