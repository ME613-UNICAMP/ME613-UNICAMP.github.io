...
my_fn <- function(data, mapping, method="loess", ...){
      ...
      geom_point() + 
      geom_smooth(method=method, ...) + theme_bw()
      p
    }
...
```{r, echo = FALSE, fig.height=4.5, fig.width=10, results = 'show', message=FALSE, warning=FALSE}
dat = data.frame(y = y, x1 = x1, x2 = x2, ey = resid(lm(y ~ x2)), ex1 = resid(lm(x1 ~ x2)))
library(ggplot2)
g = ggplot(dat, aes(y = y, x = x1, colour = x2))
g = g + geom_point(colour="grey50", size = 5) + geom_smooth(method = lm, se = FALSE, colour = "black") 
g = g + geom_point(size = 4)  + theme_bw()
g
```
...
g2 = g2 + geom_point(colour="grey50", size = 5) + geom_smooth(method = lm, se = FALSE, colour = "black") + geom_point(size = 4) +ylab("resid(lm(y ~ x2))")+xlab("resid(lm(x1 ~ x2))") + theme_bw()
g2
```
...
g = g + xlab("% in Agriculture") + ylab("Fertility") + theme_bw()
g
```
